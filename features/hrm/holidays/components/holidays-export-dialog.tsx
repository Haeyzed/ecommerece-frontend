...
import { useHolidaysExport } from '@/features/hrm/holidays/api'
import { holidayExportSchema, type HolidayExportFormData } from '@/features/hrm/holidays/schemas'
...
type HolidaysExportDialogProps = {
  open: boolean
...
export function HolidaysExportDialog({
  ...
  const form = useForm<HolidayExportFormData>({
    resolver: zodResolver(holidayExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
  ...
  const onSubmit = (data: HolidayExportFormData) => {
    exportHolidays(
      {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="text-start">
            <DialogTitle>Export Holidays</DialogTitle>
            <DialogDescription>
              Select export format, method, and columns. {ids.length > 0 && `${ids.length} holiday(s) selected.`}
            </DialogDescription>
          </DialogHeader>
    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          <DrawerTitle>Export Holidays</DrawerTitle>
          <DrawerDescription>
            Select export format, method, and columns. {ids.length > 0 && `${ids.length} holiday(s) selected.`}
          </DrawerDescription>
        </DrawerHeader>

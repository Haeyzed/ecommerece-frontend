...
import { useShiftsExport } from '@/features/hrm/shifts/api'
import { shiftExportSchema, type ShiftExportFormData } from '@/features/hrm/shifts/schemas'
...
const AVAILABLE_COLUMNS = [
  ...
  { value: 'grace_in', label: 'Grace In (mins)' },
  { value: 'grace_out', label: 'Grace Out (mins)' },
  { value: 'is_active', label: 'Status' },
  { value: 'created_at', label: 'Date Created' },
  { value: 'updated_at', label: 'Last Updated' },
] as const

type ShiftsExportDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  ids?: number[]
}

export function ShiftsExportDialog({
                                     open,
                                     onOpenChange,
                                     ids = [],
                                   }: ShiftsExportDialogProps) {
  const isDesktop = useMediaQuery('(min-width: 768px)')
  const { mutate: exportShifts, isPending } = useShiftsExport()
  const { api } = useApiClient()

  const form = useForm<ShiftExportFormData>({
    resolver: zodResolver(shiftExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
      columns: ['id', 'name', 'start_time', 'end_time', 'total_hours', 'is_active'],
  ...
  const onSubmit = (data: ShiftExportFormData) => {
    exportShifts(
      {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="text-start">
            <DialogTitle>Export Shifts</DialogTitle>
            <DialogDescription>
              Select export format, method, and columns. {ids.length > 0 && `${ids.length} shift(s) selected.`}
            </DialogDescription>
          </DialogHeader>
    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          <DrawerTitle>Export Shifts</DrawerTitle>
          <DrawerDescription>
            Select export format, method, and columns. {ids.length > 0 && `${ids.length} shift(s) selected.`}
          </DrawerDescription>
        </DrawerHeader>

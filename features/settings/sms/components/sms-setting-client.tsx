...
} from '@/features/settings/sms/api'
...
export function SmsSettingClient() {
  ...
  useEffect(() => {
    if (isError && error) {
      toast.error(
        error instanceof Error ? error.message : 'Failed to load SMS providers'
      )
    }
  }, [isError, error])
      ...
      <Main className='flex flex-1 flex-col gap-4 sm:gap-6'>
        <div>
          ...
          <p className='text-muted-foreground'>
            Configure SMS gateways (Twilio, Tonkra, Revesms, etc.). Select a
            gateway and set credentials.
          </p>
        </div>

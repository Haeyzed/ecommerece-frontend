...
export function CustomerGroupsActionDialog(
  props: CustomerGroupsActionDialogProps
) {
  ...
  const defaultPercentage =
    currentRow && typeof currentRow.percentage === 'number'
      ? currentRow.percentage
      : currentRow
        ? parseFloat(String(currentRow.percentage)) || 0
        : 0
  const form = useForm<CustomerGroupFormData>({
    resolver: zodResolver(
      customerGroupSchema
    ) as Resolver<CustomerGroupFormData>,
    defaultValues:
      isEdit && currentRow
        ? {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className='sm:max-w-lg'>
          <DialogHeader className='text-start'>
            <DialogTitle>
              {isEdit ? 'Edit Customer Group' : 'Add Customer Group'}
            </DialogTitle>
            <DialogDescription>
              {isEdit ? 'Update details.' : 'Create a new customer group.'}
            </DialogDescription>
          </DialogHeader>

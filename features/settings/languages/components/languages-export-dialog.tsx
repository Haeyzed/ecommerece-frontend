...
import { useLanguagesExport } from '@/features/settings/languages/api'
import { languageExportSchema, type LanguageExportFormData } from '@/features/settings/languages/schemas'
...
const AVAILABLE_COLUMNS = [
  ...
  { value: 'dir', label: 'Text Direction' },
  { value: 'created_at', label: 'Created At' },
  { value: 'updated_at', label: 'Updated At' },
] as const

type LanguagesExportDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  ids?: number[]
}

export function LanguagesExportDialog({
                                        open,
                                        onOpenChange,
                                        ids = [],
                                      }: LanguagesExportDialogProps) {
  const isDesktop = useMediaQuery('(min-width: 768px)')
  const { mutate: exportLanguages, isPending } = useLanguagesExport()
  const { api } = useApiClient()

  const form = useForm<LanguageExportFormData>({
    resolver: zodResolver(languageExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
      columns: ['id', 'code', 'name', 'name_native', 'dir'],
  ...
  const onSubmit = (data: LanguageExportFormData) => {
    exportLanguages(
      {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="text-start">
            ...
            <DialogDescription>
              Select export format, method, and columns. {ids.length > 0 && `${ids.length} language(s) selected.`}
            </DialogDescription>
          </DialogHeader>
    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          <DrawerTitle>Export Languages</DrawerTitle>
          <DrawerDescription>
            Select export format, method, and columns. {ids.length > 0 && `${ids.length} language(s) selected.`}
          </DrawerDescription>
        </DrawerHeader>

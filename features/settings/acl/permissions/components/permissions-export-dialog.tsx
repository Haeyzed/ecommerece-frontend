...
import {
  type PermissionExportFormData,
  permissionExportSchema,
...
type PermissionsExportDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  ids?: number[]
}

export function PermissionsExportDialog({
  open,
  onOpenChange,
  ids = [],
}: PermissionsExportDialogProps) {
  const isDesktop = useMediaQuery('(min-width: 768px)')
  const { mutate: exportPermissions, isPending } = usePermissionsExport()
  const { api } = useApiClient()

  const form = useForm<PermissionExportFormData>({
    resolver: zodResolver(permissionExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
      columns: ['id', 'name', 'guard_name', 'module', 'is_active'],
  ...
  const onSubmit = (data: PermissionExportFormData) => {
    exportPermissions(
      {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className='max-h-[90vh] overflow-y-auto sm:max-w-2xl'>
          <DialogHeader className='text-start'>
            <DialogTitle>Export Permissions</DialogTitle>
            <DialogDescription>
              Select export format, method, and columns.{' '}
              {ids.length > 0 && `${ids.length} permission(s) selected.`}
            </DialogDescription>
          </DialogHeader>
    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent>
        <DrawerHeader className='text-left'>
          <DrawerTitle>Export Permissions</DrawerTitle>
          <DrawerDescription>
            Select export format, method, and columns.{' '}
            {ids.length > 0 && `${ids.length} permission(s) selected.`}
          </DrawerDescription>

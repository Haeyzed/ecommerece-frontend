...
import { useTimezonesExport } from '@/features/settings/timezones/api'
import { timezoneExportSchema, type TimezoneExportFormData } from '@/features/settings/timezones/schemas'
...
type TimezonesExportDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  ids?: number[]
}

export function TimezonesExportDialog({
                                        open,
                                        onOpenChange,
                                        ids = [],
                                      }: TimezonesExportDialogProps) {
  const isDesktop = useMediaQuery('(min-width: 768px)')
  const { mutate: exportTimezones, isPending } = useTimezonesExport()
  const { api } = useApiClient()

  const form = useForm<TimezoneExportFormData>({
    resolver: zodResolver(timezoneExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
  ...
  const onSubmit = (data: TimezoneExportFormData) => {
    exportTimezones(
      {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="text-start">
            <DialogTitle>Export Timezones</DialogTitle>
            <DialogDescription>
              Select export format, method, and columns. {ids.length > 0 && `${ids.length} timezone(s) selected.`}
            </DialogDescription>
          </DialogHeader>
    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          <DrawerTitle>Export Timezones</DrawerTitle>
          <DrawerDescription>
            Select export format, method, and columns. {ids.length > 0 && `${ids.length} timezone(s) selected.`}
          </DrawerDescription>
        </DrawerHeader>

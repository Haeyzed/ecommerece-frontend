...
import { useBulkDestroyCities } from '@/features/settings/cities/api'
...
type CitiesMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function CitiesMultiDeleteDialog<TData>({
                                                 ...
                                                 onOpenChange,
                                                 table,
                                               }: CitiesMultiDeleteDialogProps<TData>) {
  const [value, setValue] = useState('')
  const selectedRows = table.getFilteredSelectedRowModel().rows
  const selectedIds = selectedRows.map(row => (row.original as City).id)
  const { mutate: bulkDestroy, isPending } = useBulkDestroyCities()
    ...
    <ConfirmDialog
      ...
      title={
        <span className='text-destructive'>
          ...
          {selectedRows.length > 1 ? 'cities' : 'city'}
        </span>
      }
      desc={

...
type StatesExportDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  ids?: number[]
}

export function StatesExportDialog({
                                     open,
                                     onOpenChange,
                                     ids = [],
                                   }: StatesExportDialogProps) {
  const isDesktop = useMediaQuery('(min-width: 768px)')
  const { mutate: exportStates, isPending } = useStatesExport()
  const { api } = useApiClient()

  const form = useForm<StateExportFormData>({
    resolver: zodResolver(stateExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
      columns: ['id', 'name', 'country_id', 'code', 'state_code'],
  ...
  const onSubmit = (data: StateExportFormData) => {
    exportStates(
      {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="text-start">
            <DialogTitle>Export States</DialogTitle>
            <DialogDescription>
              Select export format, method, and columns. {ids.length > 0 && `${ids.length} state(s) selected.`}
            </DialogDescription>
          </DialogHeader>
    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          <DrawerTitle>Export States</DrawerTitle>
          <DrawerDescription>
            Select export format, method, and columns. {ids.length > 0 && `${ids.length} state(s) selected.`}
          </DrawerDescription>
        </DrawerHeader>

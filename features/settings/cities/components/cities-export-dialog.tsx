...
import { useCitiesExport } from '@/features/settings/cities/api'
import { cityExportSchema, type CityExportFormData } from '@/features/settings/cities/schemas'
...
type CitiesExportDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  ids?: number[]
}

export function CitiesExportDialog({
                                     open,
                                     onOpenChange,
                                     ids = [],
                                   }: CitiesExportDialogProps) {
  const isDesktop = useMediaQuery('(min-width: 768px)')
  const { mutate: exportCities, isPending } = useCitiesExport()
  const { api } = useApiClient()

  const form = useForm<CityExportFormData>({
    resolver: zodResolver(cityExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
      columns: ['id', 'name', 'country_id', 'state_id', 'country_code', 'state_code'],
  ...
  const onSubmit = (data: CityExportFormData) => {
    exportCities(
      {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="text-start">
            <DialogTitle>Export Cities</DialogTitle>
            <DialogDescription>
              Select export format, method, and columns. {ids.length > 0 && `${ids.length} city(s) selected.`}
            </DialogDescription>
          </DialogHeader>
    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          <DrawerTitle>Export Cities</DrawerTitle>
          <DrawerDescription>
            Select export format, method, and columns. {ids.length > 0 && `${ids.length} city(s) selected.`}
          </DrawerDescription>
        </DrawerHeader>

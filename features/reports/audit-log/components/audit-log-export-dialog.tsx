...
const AVAILABLE_COLUMNS = [
  ...
  { value: 'auditable_type', label: 'Model' },
  { value: 'auditable_id', label: 'Model ID' },
...
export function AuditLogExportDialog({
  ...
  const form = useForm<AuditExportFormData>({
    resolver: zodResolver(auditExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
  ...
  const onSubmit = (data: AuditExportFormData) => {
    exportAudits(
      {
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader className="text-start">
            ...
            <DialogDescription>
              ...
              {ids.length > 0 && ` ${ids.length} audit(s) selected.`}
            </DialogDescription>
          </DialogHeader>

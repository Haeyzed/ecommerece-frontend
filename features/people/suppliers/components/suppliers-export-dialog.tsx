...
type SuppliersExportDialogProps = {
  open: boolean
...
export function SuppliersExportDialog({
  ...
  const form = useForm<SupplierExportFormData>({
    resolver: zodResolver(supplierExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
  ...
  const onSubmit = (data: SupplierExportFormData) => {
    exportSuppliers(
      {
  ...
  const handleSelectAllColumns = () => {
    form.setValue('columns', [...SUPPLIER_EXPORT_COLUMNS])
  }
  ...
  const description =
    ids.length > 0
      ? `Select export format, method, and columns. ${ids.length} supplier(s) selected.`

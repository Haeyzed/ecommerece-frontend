...
import { useLeaveTypesExport } from '@/features/hrm/leave-types/api'
import { leaveTypeExportSchema, type LeaveTypeExportFormData } from '@/features/hrm/leave-types/schemas'
...
const AVAILABLE_COLUMNS = [
  ...
  { value: 'annual_quota', label: 'Annual Quota' },
  { value: 'carry_forward_limit', label: 'Carry Forward Limit' },
  { value: 'encashable', label: 'Encashable' },
  { value: 'is_active', label: 'Status' },
  { value: 'created_at', label: 'Date Created' },
  { value: 'updated_at', label: 'Last Updated' },
] as const

type LeaveTypesExportDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  ids?: number[]
}

export function LeaveTypesExportDialog({
                                         open,
                                         onOpenChange,
                                         ids = [],
                                       }: LeaveTypesExportDialogProps) {
  const isDesktop = useMediaQuery('(min-width: 768px)')
  const { mutate: exportLeaveTypes, isPending } = useLeaveTypesExport()
  const { api } = useApiClient()

  const form = useForm<LeaveTypeExportFormData>({
    resolver: zodResolver(leaveTypeExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
      columns: ['id', 'name', 'annual_quota', 'is_active'],
  ...
  const onSubmit = (data: LeaveTypeExportFormData) => {
    exportLeaveTypes(
      {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="text-start">
            <DialogTitle>Export Leave Types</DialogTitle>
            <DialogDescription>
              Select export format, method, and columns. {ids.length > 0 && `${ids.length} leave type(s) selected.`}
            </DialogDescription>
          </DialogHeader>
    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          <DrawerTitle>Export Leave Types</DrawerTitle>
          <DrawerDescription>
            Select export format, method, and columns. {ids.length > 0 && `${ids.length} leave type(s) selected.`}
          </DrawerDescription>
        </DrawerHeader>

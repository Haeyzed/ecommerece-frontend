...
import { useLeaveTypesImport, useLeaveTypesTemplateDownload } from '@/features/hrm/leave-types'
import { leaveTypeImportSchema, type LeaveTypeImportFormData } from '@/features/hrm/leave-types'
import { LeaveTypesCsvPreviewDialog } from '@/features/hrm/leave-types'
...
type LeaveTypesImportDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function LeaveTypesImportDialog({
                                         open,
                                         onOpenChange,
                                       }: LeaveTypesImportDialogProps) {
  const isDesktop = useMediaQuery("(min-width: 768px)")
  const { mutate: importLeaveTypes, isPending } = useLeaveTypesImport()
  const { mutate: downloadTemplate, isPending: isDownloading } = useLeaveTypesTemplateDownload()

  const [previewOpen, setPreviewOpen] = useState(false)
  const [previewData, setPreviewData] = useState<any[]>([])

  const form = useForm<LeaveTypeImportFormData>({
    resolver: zodResolver(leaveTypeImportSchema),
    defaultValues: {
      file: [],
    },
  })
  ...
  const handlePreview = (data: LeaveTypeImportFormData) => {
        ...
        <Controller
          ...
          render={({ field: { value, onChange, ...fieldProps }, fieldState }) => (
            <Field data-invalid={!!fieldState.error}>
              ...
              <FieldDescription>
                Upload the file containing your leave types data.
              </FieldDescription>
        ...
        <Dialog open={open} onOpenChange={handleOpenChange}>
          <DialogContent className='sm:max-w-md'>
            <DialogHeader className='text-start'>
              ...
              <DialogDescription>
                Bulk create leave types by uploading a CSV or Excel file.
              </DialogDescription>
            </DialogHeader>

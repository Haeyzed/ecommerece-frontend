...
type WarehousesExportDialogProps = {
  open: boolean
...
export function WarehousesExportDialog({
  ...
  const form = useForm<WarehouseExportFormData>({
    resolver: zodResolver(warehouseExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
      columns: ['id', 'name', 'phone', 'email', 'address', 'is_active'],
    },
  })
  ...
  const onSubmit = (data: WarehouseExportFormData) => {
    exportWarehouses(
      {
        ...
        <DialogContent className='max-h-[90vh] overflow-y-auto sm:max-w-2xl'>
          <DialogHeader className='text-start'>
            ...
            <DialogDescription>
              ...
              {ids.length > 0 && `${ids.length} warehouse(s) selected.`}
            </DialogDescription>
          </DialogHeader>
    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent>
        <DrawerHeader className='text-left'>
          <DrawerTitle>Export Warehouses</DrawerTitle>
          <DrawerDescription>
            Select export format, method, and columns.{' '}
            {ids.length > 0 && `${ids.length} warehouse(s) selected.`}
          </DrawerDescription>

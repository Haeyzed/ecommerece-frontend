...
import { type TaxExportFormData, taxExportSchema } from '@/features/settings/taxes/schemas'
...
const AVAILABLE_COLUMNS = [
  ...
  { value: 'woocommerce_tax_id', label: 'WooCommerce ID' },
  { value: 'created_at', label: 'Date Created' },
  { value: 'updated_at', label: 'Last Updated' },
] as const

type TaxesExportDialogProps = {
  open: boolean
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="text-start">
            ...
            <DialogDescription>
              Select export format, method, and columns. {ids.length > 0 && `${ids.length} tax(es) selected.`}
            </DialogDescription>
          </DialogHeader>
    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          <DrawerTitle>Export Taxes</DrawerTitle>
          <DrawerDescription>
            Select export format, method, and columns. {ids.length > 0 && `${ids.length} tax(es) selected.`}
          </DrawerDescription>
        </DrawerHeader>

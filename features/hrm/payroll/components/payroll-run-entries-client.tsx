...
export function PayrollRunEntriesClient({
  runId,
}: PayrollRunEntriesClientProps) {
  const { data: run, isLoading: runLoading } = usePayrollRun(runId)
  const { data: entriesData, isLoading: entriesLoading } = usePayrollRunEntries(
    runId,
    { per_page: 50 }
  )
          ...
          <div className='flex items-center gap-2'>
            ...
            <div>
              ...
              <p className='text-muted-foreground'>
                {run?.status
                  ? `Status: ${run.status}`
                  : 'Entries and payslips for this run.'}
              </p>
            </div>
          </div>
          <Button
              ...
              <TableBody>
                {entries.length === 0 ? (
                  <TableRow>
                    ...
                    >
                      No entries yet. Click Regenerate entries to generate
                      payslips.
                    </TableCell>
                  </TableRow>
                ) : (
                  entries.map((entry) => (
                    <TableRow key={entry.id}>
                      ...
                      <TableCell className='text-right'>
                        {Number(entry.gross_salary).toFixed(2)}
                      </TableCell>
                      <TableCell className='text-right'>
                        {Number(entry.total_deductions).toFixed(2)}
                      </TableCell>
                      <TableCell className='text-right'>
                        {Number(entry.net_salary).toFixed(2)}
                      </TableCell>

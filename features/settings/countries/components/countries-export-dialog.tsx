...
import { useCountriesExport } from '@/features/settings/countries/api'
import { countryExportSchema, type CountryExportFormData } from '@/features/settings/countries/schemas'
...
const AVAILABLE_COLUMNS = [
  ...
  { value: 'phone_code', label: 'Phone Code' },
  { value: 'region', label: 'Region' },
  { value: 'subregion', label: 'Subregion' },
  { value: 'native', label: 'Native Name' },
...
type CountriesExportDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  ids?: number[]
}

export function CountriesExportDialog({
                                        open,
                                        onOpenChange,
                                        ids = [],
                                      }: CountriesExportDialogProps) {
  const isDesktop = useMediaQuery('(min-width: 768px)')
  const { mutate: exportCountries, isPending } = useCountriesExport()
  const { api } = useApiClient()

  const form = useForm<CountryExportFormData>({
    resolver: zodResolver(countryExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
      columns: ['id', 'name', 'iso2', 'iso3', 'phone_code', 'status'],
  ...
  const onSubmit = (data: CountryExportFormData) => {
    exportCountries(
      {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="text-start">
            <DialogTitle>Export Countries</DialogTitle>
            <DialogDescription>
              Select export format, method, and columns. {ids.length > 0 && `${ids.length} country(s) selected.`}
            </DialogDescription>
          </DialogHeader>
    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          <DrawerTitle>Export Countries</DrawerTitle>
          <DrawerDescription>
            Select export format, method, and columns. {ids.length > 0 && `${ids.length} country(s) selected.`}
          </DrawerDescription>
        </DrawerHeader>

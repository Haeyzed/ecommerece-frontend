...
import { useDocumentTypesExport } from '@/features/hrm/document-types/api'
import { type DocumentTypeExportFormData, documentTypeExportSchema } from '@/features/hrm/document-types/schemas'
...
const AVAILABLE_COLUMNS = [
  ...
  { value: 'requires_expiry', label: 'Requires Expiry' },
  { value: 'is_active', label: 'Status' },
  { value: 'created_at', label: 'Date Created' },
  { value: 'updated_at', label: 'Last Updated' },
] as const

type DocumentTypesExportDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  ids?: number[]
}

export function DocumentTypesExportDialog({
                                            open,
                                            onOpenChange,
                                            ids = [],
                                          }: DocumentTypesExportDialogProps) {
  const isDesktop = useMediaQuery('(min-width: 768px)')
  const { mutate: exportDocumentTypes, isPending } = useDocumentTypesExport()
  const { api } = useApiClient()

  const form = useForm<DocumentTypeExportFormData>({
    resolver: zodResolver(documentTypeExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
      columns: ['id', 'name', 'code', 'requires_expiry', 'is_active'],
  ...
  const onSubmit = (data: DocumentTypeExportFormData) => {
    exportDocumentTypes(
      {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="text-start">
            ...
            <DialogDescription>
              Select export format, method, and columns. {ids.length > 0 && `${ids.length} document type(s) selected.`}
            </DialogDescription>
          </DialogHeader>

...
import { useUnitsImport, useUnitsTemplateDownload } from '@/features/products/units/api'
import { type UnitImportFormData, unitImportSchema } from '@/features/products/units/schemas'
...
export function UnitsImportDialog({
  ...
  const { mutate: importUnits, isPending } = useUnitsImport()
  const { mutate: downloadTemplate, isPending: isDownloading } = useUnitsTemplateDownload()

  const [previewOpen, setPreviewOpen] = useState(false)
  const [previewData, setPreviewData] = useState<any[]>([])

  const form = useForm<UnitImportFormData>({
    resolver: zodResolver(unitImportSchema),
    defaultValues: {
      file: [],
    },
  })
  ...
  const handlePreview = (data: UnitImportFormData) => {
      ...
      <FieldGroup>
        <div className="space-y-2 rounded-md border bg-muted/50 p-3 text-sm">
          ...
          <ul className="list-disc list-inside space-y-1 text-muted-foreground">
            ...
            <li><code className="rounded bg-background px-1 py-0.5 text-xs">code*</code> - Short code</li>
          </ul>
        </div>
        <Controller
          ...
          render={({ field: { value, onChange, ...fieldProps }, fieldState }) => (
            <Field data-invalid={!!fieldState.error}>
              ...
              <FieldDescription>
                Upload the file containing your unit data.
              </FieldDescription>
        ...
        <Dialog open={open} onOpenChange={handleOpenChange}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader className="text-start">
              ...
              <DialogDescription>
                Bulk create units by uploading a CSV or Excel file.
              </DialogDescription>
            </DialogHeader>

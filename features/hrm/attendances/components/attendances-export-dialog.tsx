...
import { useAttendancesExport } from '@/features/hrm/attendances/api'
import { attendanceExportSchema, type AttendanceExportFormData } from '@/features/hrm/attendances/schemas'
...
type AttendancesExportDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  ids?: number[]
}

export function AttendancesExportDialog({
                                          open,
                                          onOpenChange,
                                          ids = [],
                                        }: AttendancesExportDialogProps) {
  const isDesktop = useMediaQuery('(min-width: 768px)')
  const { mutate: exportAttendances, isPending } = useAttendancesExport()
  const { api } = useApiClient()

  const form = useForm<AttendanceExportFormData>({
    resolver: zodResolver(attendanceExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
      columns: ['id', 'employee_name', 'date', 'checkin', 'checkout', 'status'],
  ...
  const onSubmit = (data: AttendanceExportFormData) => {
    exportAttendances(
      {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="text-start">
            <DialogTitle>Export Attendances</DialogTitle>
            <DialogDescription>
    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          <DrawerTitle>Export Attendances</DrawerTitle>
          <DrawerDescription>
            Select export format, method, and columns. {ids.length > 0 && `${ids.length} record(s) selected.`}
          </DrawerDescription>
        </DrawerHeader>

...
import { useRolesExport } from '@/features/settings/acl/roles/api'
import { roleExportSchema, type RoleExportFormData } from '@/features/settings/acl/roles/schemas'
...
const AVAILABLE_COLUMNS = [
  ...
  { value: 'guard_name', label: 'Guard Name' },
  { value: 'is_active', label: 'Status' },
  { value: 'created_at', label: 'Date Created' },
  { value: 'updated_at', label: 'Last Updated' },
] as const

type RolesExportDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  ids?: number[]
}

export function RolesExportDialog({
                                    open,
                                    onOpenChange,
                                    ids = [],
                                  }: RolesExportDialogProps) {
  const isDesktop = useMediaQuery('(min-width: 768px)')
  const { mutate: exportRoles, isPending } = useRolesExport()
  const { api } = useApiClient()

  const form = useForm<RoleExportFormData>({
    resolver: zodResolver(roleExportSchema),
    defaultValues: {
      format: 'excel',
      method: 'download',
      columns: ['id', 'name', 'guard_name', 'is_active'],
  ...
  const onSubmit = (data: RoleExportFormData) => {
    exportRoles(
      {
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="text-start">
            ...
            <DialogDescription>
              Select export format, method, and columns. {ids.length > 0 && `${ids.length} role(s) selected.`}
            </DialogDescription>
          </DialogHeader>
    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent>
        <DrawerHeader className="text-left">
          <DrawerTitle>Export Roles</DrawerTitle>
          <DrawerDescription>
            Select export format, method, and columns. {ids.length > 0 && `${ids.length} role(s) selected.`}
          </DrawerDescription>
        </DrawerHeader>

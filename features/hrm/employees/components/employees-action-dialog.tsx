...
import { CancelCircleIcon, CloudUploadIcon, Delete02Icon, File02Icon, PlusSignIcon } from '@hugeicons/core-free-icons'
...
import { type EmployeeFormData, employeeSchema } from '@/features/hrm/employees/schemas'
      ...
      <form id={id} onSubmit={form.handleSubmit(onSubmit)} className={cn('space-y-8', className)}>
            ...
            <Controller control={form.control} name="image" render={({ field: { value, onChange }, fieldState }) => {
              ...
              return (
                <Field data-invalid={!!fieldState.error} className="col-span-1 md:col-span-2 mt-2">
                  ...
                  {existingImageUrl && !hasNewImage && (
                    <div className="mb-3 flex items-center gap-3 rounded-md border p-3">
                      <div className="relative size-16 overflow-hidden rounded-md bg-muted">
                        <ImageZoom
                          backdropClassName={cn(resolvedTheme === 'dark' ? '[&_[data-rmiz-modal-overlay="visible"]]:bg-white/80' : '[&_[data-rmiz-modal-overlay="visible"]]:bg-black/80')}>
                          <Image src={existingImageUrl} alt="Profile" width={64} height={64}
                  ...
                  <FileUpload value={value} onValueChange={onChange} accept="image/*" maxFiles={1}
                              maxSize={5 * 1024 * 1024} onFileReject={(_, m) => form.setError('image', { message: m })}>
                    <FileUploadDropzone className="flex-row flex-wrap border-dotted text-center">
                      <HugeiconsIcon icon={CloudUploadIcon} className="size-4" />
                      Drag and drop or
                      <FileUploadTrigger asChild><Button variant="link" size="sm" className="p-0">choose
                        file</Button></FileUploadTrigger>
                    </FileUploadDropzone>
                    <FileUploadList>
                      {value?.map((file: any, i: number) => (
                        <FileUploadItem key={i} value={file}>
                          ...
                          <FileUploadItemDelete asChild>
                            <Button variant="ghost" size="icon"><HugeiconsIcon icon={CancelCircleIcon}
        ...
        <div className="space-y-4 rounded-xl border p-4 bg-card">
          ...
          <FieldGroup className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Field data-invalid={!!fieldState.error} className="flex flex-col">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {canCreateDesignation && (
                    <Button type="button" size="icon" variant="outline"
                            onClick={() => setIsDesignationOpen(true)}><HugeiconsIcon icon={PlusSignIcon}
                                                                                      className="size-4" /></Button>
                  )}
                </div>
                {fieldState.error && <FieldError errors={[fieldState.error]} />}
            ...
            <Controller control={form.control} name="shift_id" render={({ field, fieldState }) => (
              <Field data-invalid={!!fieldState.error} className="flex flex-col">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {canCreateShift && (
                    <Button type="button" size="icon" variant="outline"
                            onClick={() => setIsShiftOpen(true)}><HugeiconsIcon icon={PlusSignIcon}
                                                                                className="size-4" /></Button>
                  )}
                </div>
                {fieldState.error && <FieldError errors={[fieldState.error]} />}
            ...
            <Controller control={form.control} name="is_active" render={({ field, fieldState }) => (
              <Field data-invalid={!!fieldState.error}
                     className="col-span-1 md:col-span-2 flex flex-row items-center justify-between rounded-md border p-4">
        ...
        <div className="space-y-4 rounded-xl border p-4 bg-card">
          ...
          <FieldGroup className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Controller control={form.control} name="user.roles" render={({ field, fieldState }) => {
              const selectedItems = field.value ? field.value.map((id) => optionRoles?.find((opt) => opt.value === id)).filter((opt): opt is {
                value: number;
                label: string
              } => !!opt) : []
              return (
                <Field data-invalid={!!fieldState.error} className="flex flex-col col-span-1 md:col-span-2">
                  ...
                  <Combobox multiple items={optionRoles || []} itemToStringLabel={(item) => item.label}
                            value={selectedItems} onValueChange={(items) => field.onChange(items.map((i) => i.value))}
                            isItemEqualToValue={(a, b) => a?.value === b?.value}>
                    ...
                    <ComboboxContent anchor={rolesAnchor}>
                      ...
                      <ComboboxList>{(item) => <ComboboxItem key={item.value}
                                                             value={item}>{item.label}</ComboboxItem>}</ComboboxList>
                    </ComboboxContent>
                  </Combobox>
                  {fieldState.error && <FieldError errors={[fieldState.error]} />}
                </Field>
            ...
            <Controller control={form.control} name="user.permissions" render={({ field, fieldState }) => {
              const selectedItems = field.value ? field.value.map((id) => optionPermissions?.find((opt) => opt.value === id)).filter((opt): opt is {
                value: number;
                label: string
              } => !!opt) : []
              return (
                <Field data-invalid={!!fieldState.error} className="flex flex-col col-span-1 md:col-span-2">
                  ...
                  <Combobox multiple items={optionPermissions || []} itemToStringLabel={(item) => item.label}
                            value={selectedItems} onValueChange={(items) => field.onChange(items.map((i) => i.value))}
                            isItemEqualToValue={(a, b) => a?.value === b?.value}>
              ...
              <div className="space-y-3 border p-3 rounded-md bg-muted/20">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <Button type="button" variant="outline" size="sm"
                          onClick={() => appendSalesTarget({ sales_from: 0, sales_to: 0, percent: 0 })}>
                ...
                {salesTargets.map((field, index) => (
                  <div key={field.id} className="grid grid-cols-12 gap-3 items-end">
                    <div className="col-span-4">
                      <Controller control={form.control} name={`sales_target.${index}.sales_from`}
                                  render={({ field: f }) => (
                                    <Field>
                    ...
                    <div className="col-span-4">
                      <Controller control={form.control} name={`sales_target.${index}.sales_to`}
                                  render={({ field: f }) => (
                                    <Field>
                    ...
                    <div className="col-span-3">
                      <Controller control={form.control} name={`sales_target.${index}.percent`}
                                  render={({ field: f }) => (
                                    <Field>
                                      {index === 0 && <FieldLabel className="text-xs">Percent (%)</FieldLabel>}
                                      <Input type="number" min={0} max={100} step="0.01" {...f}
                                             onChange={e => f.onChange(Number(e.target.value))} />
                                    </Field>
                                  )} />
                    </div>
                ...
                {form.formState.errors.sales_target && <p
                  className="text-sm font-medium text-destructive">{form.formState.errors.sales_target.root?.message}</p>}
              </div>
            </FieldGroup>
          )}
        </div>
        ...
        <div className="space-y-4 rounded-xl border p-4 bg-card">
          <div className="border-b pb-2 mb-4 flex items-center justify-between">
            ...
            <Button type="button" variant="outline" size="sm" onClick={() => appendDocument({
              document_type_id: 0,
              name: '',
              notes: '',
              issue_date: '',
              ...
              return (
                <div key={docField.id} className="relative p-4 border rounded-md bg-muted/10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <Controller control={form.control} name={`documents.${index}.document_type_id`}
                                render={({ field, fieldState }) => (
                                  <Field data-invalid={!!fieldState.error} className="flex flex-col">
                                    <FieldLabel>Document Type <span className="text-destructive">*</span></FieldLabel>
                                    <Combobox items={optionDocumentTypes || []} itemToStringLabel={(i) => i.label}
                                              value={(optionDocumentTypes || []).find((d) => d.value === field.value) ?? null}
                                              onValueChange={(i) => field.onChange(i?.value ?? 0)}>
                    ...
                    <Controller control={form.control} name={`documents.${index}.name`}
                                render={({ field, fieldState }) => (
                                  <Field data-invalid={!!fieldState.error}>
                                    <FieldLabel>Document Name</FieldLabel>
                                    <Input placeholder="e.g. Identity Card Scanned" {...field}
                                           value={field.value || ''} />
                                    {fieldState.error && <FieldError errors={[fieldState.error]} />}
                                  </Field>
                                )} />

                    <Controller control={form.control} name={`documents.${index}.issue_date`}
                                render={({ field, fieldState }) => (
                                  <Field data-invalid={!!fieldState.error} className="flex flex-col">
                    ...
                    {selectedType?.requires_expiry && (
                      <Controller control={form.control} name={`documents.${index}.expiry_date`}
                                  render={({ field, fieldState }) => (
                                    <Field data-invalid={!!fieldState.error} className="flex flex-col">
                    ...
                    <Controller control={form.control} name={`documents.${index}.notes`}
                                render={({ field, fieldState }) => (
                                  <Field data-invalid={!!fieldState.error} className="md:col-span-2">
                                    <FieldLabel>Notes</FieldLabel>
                                    <Textarea placeholder="Any notes..." className="resize-none h-16" {...field}
                                              value={field.value || ''} />
                                    {fieldState.error && <FieldError errors={[fieldState.error]} />}
                                  </Field>
                                )} />

                    <Controller control={form.control} name={`documents.${index}.file`}
                                render={({ field: { value, onChange }, fieldState }) => {
                                  ...
                                  return (
                                    <Field data-invalid={!!fieldState.error} className="col-span-1 md:col-span-2">
                                      <FieldLabel>File Upload {isEditDocMode && !hasNewFile &&
                                        <span className="text-muted-foreground font-normal ml-2">(Existing file is kept unless replaced)</span>}</FieldLabel>
                                      <FileUpload value={value} onValueChange={onChange} maxFiles={1}
                                                  maxSize={5 * 1024 * 1024}>
                                        <FileUploadDropzone
                                          className="flex-row flex-wrap border-dotted py-4 text-center">
                                          <HugeiconsIcon icon={CloudUploadIcon} className="size-4" />
                                          Drag and drop or
                                          <FileUploadTrigger asChild><Button variant="link" size="sm" className="p-0">choose
                                            file</Button></FileUploadTrigger>

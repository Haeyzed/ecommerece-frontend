...
import { type RoleImportFormData, roleImportSchema } from '@/features/settings/acl/roles/schemas'
...
type RolesImportDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function RolesImportDialog({
                                    open,
                                    onOpenChange,
                                  }: RolesImportDialogProps) {
  const isDesktop = useMediaQuery('(min-width: 768px)')
  const { mutate: importRoles, isPending } = useRolesImport()
  const { mutate: downloadTemplate, isPending: isDownloading } = useRolesTemplateDownload()

  const [previewOpen, setPreviewOpen] = useState(false)
  const [previewData, setPreviewData] = useState<any[]>([])

  const form = useForm<RoleImportFormData>({
  ...
  const handlePreview = (data: RoleImportFormData) => {
      ...
      <FieldGroup>
        <div className="space-y-2 rounded-md border bg-muted/50 p-3 text-sm">
          ...
          <ul className="list-disc list-inside space-y-1 text-muted-foreground">
            <li><code className="rounded bg-background px-1 py-0.5 text-xs">name*</code> - Role name</li>
          </ul>
        ...
        <Controller
          ...
          render={({ field: { value, onChange, ...fieldProps }, fieldState }) => (
            <Field data-invalid={!!fieldState.error}>
              ...
              <FieldDescription>
                Upload the file containing your roles data.
              </FieldDescription>
        ...
        <Dialog open={open} onOpenChange={handleOpenChange}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader className="text-start">
              ...
              <DialogDescription>
                Bulk create roles by uploading a CSV or Excel file.
              </DialogDescription>
            </DialogHeader>
        ...
        <Drawer open={open} onOpenChange={handleOpenChange}>
          <DrawerContent>
            <DrawerHeader className="text-left">
              <DrawerTitle>Import Roles</DrawerTitle>
              <DrawerDescription>
                Bulk create roles by uploading a CSV or Excel file.
              </DrawerDescription>
            </DrawerHeader>
